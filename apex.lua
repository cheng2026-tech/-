...
local CONFIG={
    ...
    L={BRAND="NEXUS",SUBTITLE="HUB · v8.1.0",CAT_MOVE="【移动】",CAT_VISION="【视觉】",CAT_UTIL="【辅助】",
        ...
        REJOIN="重新加入",RESET="重置",SHUTDOWN="关闭",ON="开",OFF="关",LOADING="CORE HUB 加载中"}}
...
local errorCount={}
local function safe(name,fn) local ok,e=pcall(fn); if not ok then errorCount[name]=(errorCount[name] or 0)+1; if errorCount[name]<=10 then warn("[NEXUS:"..name.."]",e) end else errorCount[name]=0 end end
...
-- ★ 关键修复：BindClick 用 InputBegan（触屏通用）
