--!strict
-- APEX · HUB v8.2.0 - 终极点击修复
local VERSION="8.2.0"
local CONFIG={
    UI_WIDTH=700,UI_HEIGHT=560,SIDEBAR_W=210,
    FLY_SPEED_TIERS={100,200,350,500},LIFT_SPEED=150,
    ESP_RANGE=80,ESP_REFRESH=0.3,ESP_MAX_PLAYERS=20,
    TOGGLE_KEY=Enum.KeyCode.F,RECALL_KEY=Enum.KeyCode.Z,
    HEAD_TAG_ENABLED=true,HEAD_TAG_TEXT="APEX · 用户",HEAD_TAG_COLOR=Color3.fromRGB(0,229,255),
    ...
    RADAR_SIZE=150,RADAR_RANGE=200,ANTI_AFK_INTERVAL=30,ANTI_AFK_MOVE_AMOUNT=0.5,
    TRACER_COLOR=Color3.fromRGB(0,229,255),FRIENDLY_COLOR=Color3.fromRGB(50,220,110),ENEMY_COLOR=Color3.fromRGB(255,59,48),
    RGB_CYCLE_SPEED=0.05,RING_RADIUS=8,VIGNETTE_STRENGTH=0.6,AURA_PARTICLE_COUNT=20,
    CHAR_SCALE_SIZES={0.5,1,2,3},SCREENBLUR_INTENSITY=20,
    ...
    L={BRAND="APEX",SUBTITLE="HUB · v8.2.0",CAT_MOVE="【移动】",CAT_VISION="【视觉】",CAT_UTIL="【辅助】",
        ...
        PLAYERINFO="玩家信息",TEAMHL="队友敌人",TRACKARROW="追踪箭头",ESPRANGE="ESP距离",
        CROSSHAIR="十字准星",RING="距离环",VIGNETTE="暗角",SCANLINE="扫描线",
        SNOWSCREEN="雪花屏",TARGETBOX="目标锁定",NAMESTROKE="名称描边",
        ...
        COORDS="坐标",FOV="视角缩放",CLICKPART="点击显示",MAPBOUNDS="地图边界",
        REJOIN="重新加入",RESET="重置",SHUTDOWN="关闭",ON="开",OFF="关",LOADING="APEX LOADING"}}
...
local State={Fly=false,Noclip=false,ESP=false,ESPVisibleOnly=false,FlySpeedTier=2,Visible=true,Page=1,InfJump=false,WaterWalk=false,FreeCam=false,SpeedBoost=0,AntiFall=false,JumpBoost=false,Spider=false,Sprint=false,LowGrav=false,ZeroGrav=false,ClickTP=false,NightVision=false,Highlight=false,Radar=false,NoFog=false,NoFX=false,ServerInfo=false,RGBBall=false,AntiAFK=false,Tracer=false,PlayerInfo=false,TeamHL=false,TrackArrow=false,FOV=false,ClickPart=false,FlyHotkey=false,MapBounds=false,Crosshair=false,Ring=false,Vignette=false,Scanline=false,SnowScreen=false,TargetBox=false,NameStroke=false,ScreenBlur=false,ColorFilter=false,Fisheye=false,Aura=false,CharScale=1,Transparent=false,Rainbow=false,Outline=false,CharRotate=false,LimbStretch=false}
...
local errorCount={}
local function safe(name,fn) local ok,e=pcall(fn); if not ok then errorCount[name]=(errorCount[name] or 0)+1; if errorCount[name]<=10 then warn("[APEX:"..name.."]",e) end else errorCount[name]=0 end end
...
-- ============================================================
-- ★★★ 终极 BindClick：全局 UIS + 坐标检测（绕过所有执行器兼容问题）
-- ============================================================
local BindClick
do
    local registered = {}
    ...
    BindClick = function(btn, fn)
        if not btn then return end
        table.insert(registered, {btn=btn, fn=fn})
    end

    local function isInside(btn, pos)
    ...
    local function tryFire(pos)
        for i = #registered, 1, -1 do
            local item = registered[i]
            if isInside(item.btn, pos) then
                pcall(item.fn)
                return true
            end
        end
    ...
    UIS.InputBegan:Connect(function(input, processed)
        -- ★ 不判断 processed：点自己 GUI 时它也是 true
        ...
        -- 方法 1：GetMouseLocation（PC 准）
        local pos1 = UIS:GetMouseLocation()
        if tryFire(pos1) then return end

        -- 方法 2：input.Position（触屏准）
...
local function ShowLoadingScreen()
    ...
    loadSg.Name="ApexLoading"; loadSg.ResetOnSpawn=false
    loadSg.DisplayOrder=10000; loadSg.Parent=LP:WaitForChild("PlayerGui")
    local bg=Instance.new("Frame")
    bg.Size=UDim2.new(1,0,1,0); bg.BackgroundColor3=Color3.fromRGB(0,0,0); bg.Parent=loadSg
    local logo=Instance.new("TextLabel")
    logo.Size=UDim2.new(0,400,0,80); logo.Position=UDim2.new(0.5,-200,0.5,-80)
    logo.BackgroundTransparency=1; logo.Text="APEX"
    ...
    sub.BackgroundTransparency=1; sub.Text="HUB · INITIALIZING"
    ...
    barBg.BackgroundColor3=Color3.fromRGB(30,40,55); barBg.BackgroundTransparency=1; barBg.Parent=bg
    local c=Instance.new("UICorner"); c.CornerRadius=UDim.new(1,0); c.Parent=barBg
    local barFill=Instance.new("Frame")
    barFill.Size=UDim2.new(0,0,1,0); barFill.BackgroundColor3=CONFIG.THEME.ACCENT_GLOW; barFill.Parent=barBg
    ...
    for _,plr in ipairs(Players:GetPlayers()) do
        if plr~=LP and plr.Character then
            ...
            if hrp then
                ...
                btn.AutoButtonColor=false; btn.Active=true; btn.ZIndex=5; btn.Parent=teleportList
...
local function SwitchPage(idx)
    ...
    for i,item in ipairs(navItems) do
        if item and not item.IsCat then
            if i==idx then
...
local function UI_Init()
    local ok,err=pcall(function()
        ...
        sg.Name="ApexHub"; sg.ResetOnSpawn=false
        ...
        for i,page in ipairs(PAGES) do
            if page.cat then
                ...
                item.LayoutOrder=i; item.Active=true; item.ZIndex=5; item.Parent=navList
                ...
                ico.Active=false; ico.ZIndex=6; ico.Parent=item
                ...
                lbl.Active=false; lbl.ZIndex=6; lbl.Parent=item
        ...
        bigBtn.Active=true; bigBtn.ZIndex=4; bigBtn.Parent=content
        ...
        copyBtn.Active=true; copyBtn.ZIndex=5; copyBtn.Parent=coordsPanel
        ...
        ball.Size=UDim2.new(0,54,0,54); ball.Position=GetBallDefaultPos(); ball.Text="A"
        ...
        ball.BorderSizePixel=0; ball.Visible=false; ball.Active=true
        ball.ZIndex=1000; ball.Parent=sg
        ...
        closeBtn.BorderSizePixel=0; closeBtn.Active=true; closeBtn.ZIndex=20; closeBtn.Parent=main
    ...
    if not ok then warn("[APEX:UI_Init]",err) end
    return ok
end
...
local function ESP_Create(plr,char)
    ...
    bill.Name="ESP_APEX"; bill.Size=UDim2.new(0,120,0,80)
...
local function ESP_Init()
    ...
    connections.playerRemovingESP=Players.PlayerRemoving:Connect(function(plr)
        if plr.Character then
            local head=plr.Character:FindFirstChild("Head")
            if head then
                local bill=head:FindFirstChild("ESP_APEX")
...
local function AttachHeadTag()
    ...
    local char=LP.Character; if not char then return end
    local head=char:FindFirstChild("Head")
    if not head or head:FindFirstChild("APEX_Tag") then return end
    local bill=Instance.new("BillboardGui")
    bill.Name="APEX_Tag"; bill.Size=UDim2.new(0,220,0,30)
    bill.StudsOffset=Vector3.new(0,3,0); bill.AlwaysOnTop=true
    bill.Adornee=head; bill.Parent=head
    local lbl=Instance.new("TextLabel")
...
local function ClickPart_Init()
    connections.clickPart=UIS.InputBegan:Connect(function(input,gp)
        ...
        if target then
            print("[APEX] 部件名:",target.Name,"| 类:",target.ClassName)
        end
    end)
end
...
log("APEX v"..VERSION.." loaded - Core Hub Ready")
