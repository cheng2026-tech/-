--!strict
-- NEXUS · HUB v8.0.0 - 59 功能完全体
local VERSION = "8.0.0"
local CONFIG = {
    UI_WIDTH=700, UI_HEIGHT=560, SIDEBAR_W=210,
    FLY_SPEED_TIERS={100,200,350,500}, LIFT_SPEED=150,
    ESP_RANGE=80, ESP_REFRESH=0.2,
    TOGGLE_KEY=Enum.KeyCode.F, RECALL_KEY=Enum.KeyCode.Z,
    HEAD_TAG_ENABLED=true, HEAD_TAG_TEXT="NEXUS · 用户", HEAD_TAG_COLOR=Color3.fromRGB(0,229,255),
    ...
    FLY_LIFT_OFFSET=3, JUMP_POWER=120, SPRINT_SPEED=90, LOW_GRAV=50,
    CLICK_TP_KEY=Enum.KeyCode.T, FLY_SPEED_UP_KEY=Enum.KeyCode.Q, FLY_SPEED_DOWN_KEY=Enum.KeyCode.E,
    NIGHT_BRIGHT=3, NIGHT_AMBIENT=Color3.fromRGB(190,190,190),
    RADAR_SIZE=150, RADAR_RANGE=200,
    ANTI_AFK_INTERVAL=30, ANTI_AFK_MOVE_AMOUNT=0.5,
    TRACER_COLOR=Color3.fromRGB(0,229,255), FRIENDLY_COLOR=Color3.fromRGB(50,220,110), ENEMY_COLOR=Color3.fromRGB(255,59,48),
    RGB_CYCLE_SPEED=0.05, RING_RADIUS=8, VIGNETTE_STRENGTH=0.6,
    AURA_PARTICLE_COUNT=20, CHAR_SCALE_SIZES={0.5,1,2,3}, SCREENBLUR_INTENSITY=20,
    THEME={OVERLAY_TRANSPARENCY=0.45, SIDEBAR_BG=Color3.fromRGB(10,12,18), SIDEBAR_ACTIVE=Color3.fromRGB(0,40,55),
        ACCENT=Color3.fromRGB(0,180,220), ACCENT_GLOW=Color3.fromRGB(0,229,255),
        TEXT=Color3.fromRGB(240,245,250), TEXT_DIM=Color3.fromRGB(120,140,160),
        GREEN=Color3.fromRGB(50,220,110), CARD_BG=Color3.fromRGB(18,22,30), CARD_HOVER=Color3.fromRGB(30,38,50),
        CARD_ON=Color3.fromRGB(0,180,220), CAT_TEXT=Color3.fromRGB(80,130,180), CORNER=12},
    L={BRAND="NEXUS", SUBTITLE="HUB · v8.0.0", CAT_MOVE="【移动】", CAT_VISION="【视觉】", CAT_UTIL="【辅助】",
        FLIGHT="飞行",NOCLIP="穿墙",SPEED="速度",SPEEDBOOST="速度增强",SPRINT="极速冲刺",JUMPBOOST="跳跃增强",
        INFJUMP="无限跳",FREECAM="自由视角",TELEPORT="传送",CLICKTP="点击传送",SPAWNTP="回出生点",
        WATERWALK="水上行走",SPIDER="蜘蛛侠",LOWGRAV="低重力",ZEROGRAV="零重力",FLYHOTKEY="飞行热键",
        NIGHTVIS="夜视",HIGHLIGHT="高亮玩家",RADAR="雷达",NOFOG="无雾",TRACER="追踪线",
        PLAYERINFO="玩家信息",TEAMHL="队友敌人",XRAY="X光",TRACKARROW="追踪箭头",ESPRANGE="ESP距离",
        CROSSHAIR="十字准星",HPBAR="头顶血条",RING="距离环",VIGNETTE="暗角",SCANLINE="扫描线",
        SNOWSCREEN="雪花屏",TARGETBOX="目标锁定",NAMESTROKE="名称描边",OBJECTHL="物件高亮",
        SCREENBLUR="屏幕模糊",COLORFILTER="色彩滤镜",FISHEYE="鱼眼",
        ESP="透视",ESPV="仅可见",AURA="粒子光环",CHARSCALE="角色缩放",TRANSPARENT="透明化",
        RAINBOW="彩虹",OUTLINE="发光轮廓",CHARROTATE="角色旋转",LIMBSTRETCH="四肢拉长",
        NOFX="屏蔽特效",SERVERINFO="服务器信息",RGBBALL="RGB球",ANTIAFK="反挂机",ANTIFALL="反掉落",
        COORDS="坐标",FOV="视角缩放",CAMMOUSE="相机跟鼠标",CLICKPART="点击显示",MAPBOUNDS="地图边界",
        ALLMARKED="全图标记",REJOIN="重新加入",RESET="重置",SHUTDOWN="关闭",ON="开",OFF="关",LOADING="CORE HUB 加载中"}}
...
local State={Fly=false,Noclip=false,ESP=false,ESPVisibleOnly=false,FlySpeedTier=2,Visible=true,Page=1,
    InfJump=false,WaterWalk=false,FreeCam=false,SpeedBoost=0,AntiFall=false,JumpBoost=false,
    Spider=false,Sprint=false,LowGrav=false,ZeroGrav=false,ClickTP=false,
    NightVision=false,Highlight=false,Radar=false,NoFog=false,NoFX=false,ServerInfo=false,
    RGBBall=false,AntiAFK=false,Tracer=false,PlayerInfo=false,TeamHL=false,XRay=false,
    TrackArrow=false,FOV=false,CamMouse=false,ClickPart=false,FlyHotkey=false,
    MapBounds=false,AllMarked=false,Crosshair=false,HPBar=false,Ring=false,Vignette=false,
    Scanline=false,SnowScreen=false,TargetBox=false,NameStroke=false,ObjectHL=false,
    ScreenBlur=false,ColorFilter=false,Fisheye=false,Aura=false,CharScale=1,Transparent=false,
    Rainbow=false,Outline=false,CharRotate=false,LimbStretch=false}
...
local function safe(name,fn) local ok,e=pcall(fn); if not ok then warn("[NEXUS:"..name.."]",e) end end
...
local PAGES={
    ...
    {key="XRay",icon="◈",title=CONFIG.L.XRAY,desc="看穿所有墙壁"},
    ...
    {key="HPBar",icon="♥",title=CONFIG.L.HPBAR,desc="玩家头顶血条"},
    ...
    {key="ObjectHL",icon="✦",title=CONFIG.L.OBJECTHL,desc="关键物件发光"},
...
local function SwitchPage(idx)
    ...
    elseif k=="XRay" then isOn=State.XRay; btnText=State.XRay and "关闭" or "开启"
    ...
    elseif k=="HPBar" then isOn=State.HPBar; btnText=State.HPBar and "关闭" or "开启"
    ...
    elseif k=="ObjectHL" then isOn=State.ObjectHL; btnText=State.ObjectHL and "关闭" or "开启"
    ...
    elseif k=="CamMouse" then isOn=State.CamMouse; btnText=State.CamMouse and "关闭" or "开启"
    ...
    elseif k=="AllMarked" then isOn=State.AllMarked; btnText=State.AllMarked and "关闭" or "开启"
...
local Tracer_Toggle,PlayerInfo_Toggle,TeamHL_Toggle,XRay_Toggle,TrackArrow_Toggle,ESPRange_Toggle
local Crosshair_Toggle,HPBar_Toggle,Ring_Toggle,Vignette_Toggle,Scanline_Toggle,SnowScreen_Toggle
local TargetBox_Toggle,NameStroke_Toggle,ObjectHL_Toggle,ScreenBlur_Toggle,ColorFilter_Toggle,Fisheye_Toggle
local Aura_Toggle,CharScale_Toggle,Transparent_Toggle,Rainbow_Toggle,Outline_Toggle,CharRotate_Toggle,LimbStretch_Toggle
local FOV_Toggle,CamMouse_Toggle,ClickPart_Toggle,MapBounds_Toggle,AllMarked_Toggle
...
local function OnBigBtnClick()
    ...
    elseif k=="ObjectHL" then ObjectHL_Toggle()
...
XRay_Toggle=function() State.XRay=not State.XRay end
...
HPBar_Toggle=function()
    State.HPBar=not State.HPBar
...
ObjectHL_Toggle=function()
    State.ObjectHL=not State.ObjectHL
    if State.ObjectHL then
        for _,obj in ipairs(workspace:GetDescendants()) do
            if obj:IsA("BasePart") and obj.Name~="Baseplate" and obj.Name~="Terrain" then
                local h=Instance.new("Highlight")
                h.FillColor=CONFIG.ACCENT_GLOW; h.FillTransparency=0.85
                h.OutlineColor=CONFIG.ACCENT_GLOW; h.OutlineTransparency=0.5
                h.Adornee=obj; h.Parent=obj
                table.insert(objectHLs,h)
            end
        end
    else
        for _,h in ipairs(objectHLs) do if h and h.Parent then h:Destroy() end end
        objectHLs={}
    end
end
...
CamMouse_Toggle=function() State.CamMouse=not State.CamMouse end
...
AllMarked_Toggle=function() State.AllMarked=not State.AllMarked end
...
Master_AllOff=function()
    ...
    State.Tracer=false; State.PlayerInfo=false; State.TeamHL=false; State.XRay=false
    State.TrackArrow=false; State.FOV=false; State.CamMouse=false
    State.ClickPart=false; State.FlyHotkey=false; State.MapBounds=false
    State.AllMarked=false; State.AntiAFK=false
    State.Crosshair=false; State.HPBar=false; State.Ring=false; State.Vignette=false
    State.Scanline=false; State.SnowScreen=false; State.TargetBox=false; State.NameStroke=false
    State.ObjectHL=false; State.ScreenBlur=false; State.ColorFilter=false; State.Fisheye=false
