...
-- 飞行速度
bindSlider(FSS,FSF,50,500,function(v)
    fSp=v;FS.Text="飞行速度: "..v
end)
-- 跑步速度
bindSlider(WSS,WSF,16,200,function(v)
    WS.Text="跑步速度: "..v
    local h=gH() if h then pcall(function() h.WalkSpeed=v end) end
end)
-- 跳跃力
bindSlider(JPS,JPF,50,350,function(v)
...
bindSlider(SSF,SSFF,0.5,5,function(v,p)
    local s=0.5+p*4.5
...
bindSlider(volSlider,volFill,0,100,function(v,p)
    volLabel.Text="音量: "..v.."%"
...
plr.CharacterAdded:Connect(function()
    ...
    task.wait(1)
    if currentScale~=1 then applyScale(currentScale) end
end)
